...
//Local to World
static i23dSFM::Mat4 l2w_Camera(const Mat3 & R, const Vec3 & t)
{
  //World to Local
  ...
  i23dSFM::Mat4 l2wmat = Mat4::Identity();
...
static void draw(void)
{
...

  {
    ...
    // apply view offset
    i23dSFM::Mat4 offset_w = l2w_Camera(Mat3::Identity(), Vec3(x_offset,y_offset,z_offset));
    ...
    const i23dSFM::Mat4 l2w = l2w_Camera(pose.rotation(), pose.translation());
    ...
    for (int i_cam=0; i_cam < vec_cameras.size(); ++i_cam)
    {
      ...
      if (isPinhole(cam->getType()))
      {
        ...
        const i23dSFM::Mat4 l2w_i = l2w_Camera(pose.rotation().transpose(), -trans);
        // stack it and use it
        glPushMatrix();
