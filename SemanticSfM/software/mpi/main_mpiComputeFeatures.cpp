...
//    assert(REMOTE_SEND_SERVER > 0);
    ...
    if (!bForce && stlplus::is_file(sImage_describer)&&(world_rank==SEND_SERVER||world_rank==RECV_SERVER)) {
...
//
            ...
            if (bForce || !stlplus::file_exists(sFeats[id_view]) || !stlplus::file_exists(sDescs[id_view])) {
                ...
                if (!ReadImage(sView_filenames[id_view].c_str(), &imageGray))
                    continue;
                ...
                if (!thread_pool.empty()) {
                    ...
                    MPI_Send(&id_view, 1, MPI_UNSIGNED, current_thread, IMAGE_ID, MPI_COMM_WORLD);

                    MPI_Send(imageGray.data(), imageGray.size(), MPI_UNSIGNED_CHAR, current_thread, IMAGE,
                             MPI_COMM_WORLD);
...
//            MPI_Bcast(views_info.get(),world_size*3,MPI_UNSIGNED,ROOT,MPI_COMM_WORLD);
        ...
        while (true) {
            ...
            MPI_Recv(&view_id, 1, MPI_UNSIGNED, SEND_SERVER, IMAGE_ID, MPI_COMM_WORLD, &status);
            cout << view_id << "->" << world_rank << endl;
            if (view_id == -1) {
                cout<<"接收到停止信号"<<endl;
                cout << view_id << "->" << world_rank << " ended!" << endl;
                buf[0] = -1;
                buf[1] = 0;
                MPI_Send(buf, 2, MPI_UNSIGNED, RECV_SERVER, IMAGE_ID_FEATURE_SIZE, MPI_COMM_WORLD);
                break;
            }
...
//                cout<<"接收　"<<view_id<<" -> "<<world_rank<<" 的图片数据"<<" 大小为"<<imageGray.size()<<endl;
                ...
                MPI_Recv(rec_buff.data(), img_size, MPI_UNSIGNED_CHAR, SEND_SERVER, IMAGE, MPI_COMM_WORLD, &status);



                // cout<<"image_describer info:"<<image_describer-><<endl;
                //  = Eigen::Map<Image<unsigned char>::Base>(rec_buff.data(), h, w);
                (*im) = Eigen::Map<Image<unsigned char>::Base>(&rec_buff.data()[0], h, w);

//                cout<<"成功接收　"<<world_rank<<" -> "<<view_id<<" 的数据并转换成图片,大小为"<<imageGray.size()<<endl;
                // Compute features and descriptors and export them to files
//                cout<<world_rank<< " 开始提取　"<<view_id<<" 的特征点";
                //std::cout << "No mask file for : " << sView_filename << std::endl;
                assert(image_describer != nullptr);
                assert(im!= nullptr);
...
//                cout<<view_id<<" regions info:"<<regions.get()->Type_id()<<endl;
                ...
                buf[0] = view_id;
                buf[1] = regions.get()->RegionCount();
                MPI_Send(buf, 2, MPI_UNSIGNED, RECV_SERVER, IMAGE_ID_FEATURE_SIZE, MPI_COMM_WORLD);
                int completed = 0;
                MPI_Recv(&completed, 1, MPI_INT, RECV_SERVER, CHECK_ID, MPI_COMM_WORLD, &status);
                if (completed != 0) {
                    continue;
                }
                points.resize(buf[1] * 4);
                int i = 0;
                for (auto point:regionsCasted->Features()) {

                    auto x = point.x();
                    auto y = point.y();
                    auto scale = point.scale();
                    auto angle = point.orientation();
...
//                cout<<"发送　"<<view_id<<" 的特征点"<<endl;
                MPI_Send(points.data(), buf[1] * 4, MPI_FLOAT, RECV_SERVER, FEATURE, MPI_COMM_WORLD);
//                cout<<"发送　"<<view_id<<" 的特征描述符"<<endl;
                //这里的发送用阻塞通信

                MPI_Send(regions->DescriptorRawData(), buf[1] * 128, MPI_UNSIGNED_CHAR, RECV_SERVER, DESCRIPTION,
                         MPI_COMM_WORLD);
...
//        unique_ptr<MPI_Request> completed_requests(new MPI_Request[image_nums]);
//        unique_ptr<MPI_Status> completed_statuses(new MPI_Status[image_nums]);
//        unsigned int local_finished = 0, remote_finished = 0;
        int recv_num = 0;
        set<int> finished;
        while (finished.size() < world_size - 2) {
            MPI_Status status;

            MPI_Recv(buf, 2, MPI_UNSIGNED, MPI_ANY_SOURCE, IMAGE_ID_FEATURE_SIZE, MPI_COMM_WORLD, &status);
            if (buf[0] == -1) {
                finished.insert(status.MPI_SOURCE);
                cout << "已结束线程数" << finished.size() << endl;
                continue;
            }
//            if(status.MPI_SOURCE==REMOTE_SEND_SERVER){
//                remote_finished=1;
//                cout<<"REMOTE SERVER END SIGN"<<endl;
//                continue;
//            }else if(status.MPI_SOURCE==SEND_SERVER){
//                local_finished=1;
//                cout<<" SERVER END  SIGN"<<endl;
//                continue;
//            }
            unsigned completed_thread = status.MPI_SOURCE;
            unsigned int id_view = buf[0];
            unsigned int feature_size = buf[1];
            int completed = 0;

            if (bForce || !stlplus::file_exists(sFeats[id_view]) || !stlplus::file_exists(sDescs[id_view])) {

                MPI_Send(&completed, 1, MPI_INT, completed_thread, CHECK_ID, MPI_COMM_WORLD);
                if (sImage_Describer_Method == "SIFT") {

                    unique_ptr<float> features(new float[feature_size * 4]);
                    unique_ptr<unsigned char> descs(new unsigned char[feature_size * 128]);


                    //auto i=id_view;
                    MPI_Recv(features.get(), feature_size * (4), MPI_FLOAT, completed_thread, FEATURE, MPI_COMM_WORLD,
                             &status);

//
                    MPI_Recv(descs.get(), feature_size * (128), MPI_UNSIGNED_CHAR, completed_thread, DESCRIPTION,
                             MPI_COMM_WORLD, &status);

//                    cout<<"成功接收　"<<id_view+2<<" 的描述符"<<endl;

                    cout << "成功接收　" << id_view << " 的特征点" << endl;
                    //因为views_info中保存的图片id比sfm_data里的大１，所以此处减去
                    std::ofstream ftfile(sFeats[id_view].c_str());
                    std::ofstream desfile(sDescs[id_view].c_str());
                    for (int j = 0; j < feature_size; ++j) {
                        ftfile << features.get()[4*j] << " " << features.get()[4*j + 1] << " " << features.get()[4*j + 2]
                               << " " << features.get()[4*j + 3] << endl;
                        for (int k = 0; k < 128; ++k) {
                            desfile << (int) (descs.get()[j*128+k]) << " ";
                        }
                        desfile << endl;
                    }
                    ftfile.close();
                    desfile.close();
                    cout << "成功接收来自　" << id_view << "->" << completed_thread << " 的特征" << endl;
                    recv_num++;
...
//                cout<<"成功接收　"<<id_view<<" 的特征点数目"<<endl;
            MPI_Request request;

            MPI_Isend(0, NULL, MPI_UNSIGNED, SEND_SERVER, THREAD_ID, MPI_COMM_WORLD, &request);
        ...
        cout << "recv server ended!" << endl;

        //先接收特征点数目

        //vector<unsigned int> feature_size(world_size);
        //openmp 并行
