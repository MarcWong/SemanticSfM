...
#include "i23dSFM/geometry/Similarity3.hpp"
...
void MainWindow::registerProject()
{
    ...
    iterCP != m_doc._sfm_data.control_points.end(); ++iterCP)
  {
      ...
      itObs != obs.end(); ++itObs)
    {
      ...
      if (!m_doc._sfm_data.IsPoseAndIntrinsicDefined(view))
        continue;
      const i23dSFM::cameras::IntrinsicBase * cam = m_doc._sfm_data.GetIntrinsics().at(view->id_intrinsic).get();
      const i23dSFM::geometry::Pose3 pose = m_doc._sfm_data.GetPoseOrDie(view);
      trianObj.add(
  ...
  // compute the similarity
  {
    ...
    Mat3 R;
    double S;
    if (i23dSFM::geometry::FindRTS(x1, x2, &S, &t, &R))
    {
      ...
      const i23dSFM::geometry::Similarity3 sim(geometry::Pose3(R, -R.transpose() * t/S), S);
