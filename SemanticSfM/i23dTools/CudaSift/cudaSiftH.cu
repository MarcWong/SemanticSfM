...
void PrintSiftData(SiftData &data, std::string path)
{
...
#endif
    std::ofstream feat((path + ".feat").c_str());
    std::ofstream desc((path + ".desc").c_str());
    std::cout << path + ".feat" << std::endl;
    int i, j;
    for(i = 0; i < data.numPts; i++)
    {
        double sum = 0;
        feat << h_data[i].xpos << " " << h_data[i].ypos << " " << h_data[i].scale << " " << h_data[i].orientation << "\n";
        for(j = 0; j < 128; j++)
        {
            desc << static_cast<unsigned int>(512 * h_data[i].data[j]) << " ";
            sum += h_data[i].data[j] * h_data[i].data[j];
        }
        desc << "\n";
    }
    return;
