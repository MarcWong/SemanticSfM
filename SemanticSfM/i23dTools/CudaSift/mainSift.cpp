...
int PrintMatchData(SiftData &siftData1, SiftData &siftData2, CudaImage &img);
...
int main(int argc, char **argv)
{
  ...
  cv::Mat limg, rimg;
//  cv::imread("/data/DataSet/castle_dense_large/urd/0000.png", 0).convertTo(limg, CV_32FC1);
  int l, r;
  string lfile, rfile;
  ofstream ofile("/home/gaomz/testmatch.txt");
  for(l = 1; l <= 10; l++)
  {
      stringstream ss;
      ss.clear();
      ...
      ss << 1000 + l;
      ss >> lfile;
      lfile = "A" + lfile.substr(1);
  cv::imread("/data/DataSet/cangzhou/A/" + lfile + ".JPG", 0).convertTo(limg, CV_32FC1);
  for(r = 1; r <= 10; r++)
  {
      if(l == r)
          continue;
      ...
      ss << 1000 + r;
      ss >> rfile;
      rfile = "A" + rfile.substr(1);
  cv::imread("/data/DataSet/cangzhou/A/" + rfile + ".JPG", 0).convertTo(rimg, CV_32FC1);
  ...
  float initBlur = 0.0f;
  float thresh = 4.0f;
  InitSiftData(siftData1, 23100, true, true);
  InitSiftData(siftData2, 23100, true, true);
  for (int i=0;i<1;i++) {
...
//  for (int i=0;i<1;i++)
    ...
    MatchSiftData(siftData2, siftData1);

  //PrintSiftData(siftData1, "/home/gaomz/100_7100");
  //PrintSiftData(siftData2, "/home/gaomz/100_7101");
  float homography[9];
  int numMatches;
  ...
  numMatches = PrintMatchData(siftData1, siftData2, img1);
  ofile << lfile << " " << siftData1.numPts << " " << rfile << " " << siftData2.numPts << " " << numMatches << endl;
#if 0
...
//  std::cout << std::setprecision(3);
  std::ofstream of("/home/gaomz/test.txt");
  for (int j=0;j<numPts;j++) {
    int k = sift1[j].match;
//    of << j << " " << k << std::endl;
    //std::cout << j << " " << sift1[j].match << " " << sift2[sift1[j].match].match << std::endl;
    if(j == sift2[sift1[j].match].match)
    {   // std::cout << cnt++ << std::endl;
        cnt++;
        of << j << " " << sift1[j].match << std::endl;
    }
