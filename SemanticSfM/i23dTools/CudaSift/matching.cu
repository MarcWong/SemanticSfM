...
__global__ void MatchSiftPoints2(SiftPoint *sift1, SiftPoint *sift2, float *corrData, int numPts1, int offset, int numPts2)
{
  __shared__ float siftPoints1[16*128];
  __shared__ float siftPoints2[16*128];
  const int tx = threadIdx.x;
  const int ty = threadIdx.y;
  const float *ptr1 = sift1[offset + min(numPts1-1,blockIdx.x*16 + ty)].data;
...
__global__ void FindMaxCorr(float *corrData, SiftPoint *sift1, SiftPoint *sift2, int numPts1, int offset, int corrWidth, int siftSize)
{
  ...
  if (tx==6)
    sift1[p1 + offset].score = maxScore[ty*16];
  if (tx==7)
    sift1[p1 + offset].ambiguity = maxScor2[ty*16] / (maxScore[ty*16] + 1e-6);
  if (tx==8)
    sift1[p1 + offset].match = maxIndex[ty*16];
  if (tx==9)
    sift1[p1 + offset].match_xpos = sift2[maxIndex[ty*16]].xpos;
  if (tx==10)
    sift1[p1 + offset].match_ypos = sift2[maxIndex[ty*16]].ypos;
  __syncthreads();
  //if (tx==0)
...
#endif
  int i;
//  std::cout << numPts1 << std::endl;
  for(i = 0; i < numPts1 / PART; i++){
  //    std::cout << i * PART << std::endl << PART << "*" << numPts2 << std::endl;
  float *d_corrData; 
  ...
  MatchSiftPoints2<<<blocks, threads>>>(sift1, sift2, d_corrData, PART, i * PART, numPts2);
#endif
  safeCall(cudaThreadSynchronize());
  dim3 blocksMax(iDivUp(numPts1, 16));
  dim3 threadsMax(16, 16);
  FindMaxCorr<<<blocksMax, threadsMax>>>(d_corrData, sift1, sift2, PART, i * PART, corrWidth, sizeof(SiftPoint));
  safeCall(cudaThreadSynchronize());
